...
const std::unordered_map<QString, QString> IconProvider::_icon_name_by_extension_map{{"apk", "application-apk.png"},
...
#ifndef NDEBUG
    std::for_each(std::begin(_icon_name_by_extension_map), std::end(_icon_name_by_extension_map), [](const auto& pair) { const QPixmap pixmap(_res_path + pair.second); assert(!pixmap.isNull()); });
#endif
}
